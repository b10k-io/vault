...
function FailExtendTime({ lock }: IExtendTimeDialogBasic) {
    return (
        <>
            <Dialog.Description className="my-8 text-sm">
        ...
        <>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex justify-between items-center mt-8">
                    <button type="submit" className="border py-1 px-2 text-sm enabled:hover:underline enabled:hover:bg-white enabled:hover:text-black disabled:opacity-50" disabled={!!Object.keys(errors).length}>
                        <div className="flex items-center gap-1">
                            <Spin textColor="text-black" show={state.status === "Mining"} />
                            <div>Extend</div>
                        </div>
                    </button>
                    <button onClick={onClose} className="border hover:bg-white hover:text-black py-1 px-2 text-sm">Cancel</button>
                </div>
            </form>
        </>
    )
